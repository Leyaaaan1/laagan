...
// ── Hook ──────────────────────────────────────────────────────────────────────
export function useRideShareCard({data, format = 'story', AppLogo = null} = {}) {
  const cardRef  = useRef(null);

  const [photoUri, setPhotoUri]   = useState(null);  // user-picked photo
  ...
  const triggerShare = useCallback(async () => {
    ...
    try {
      ...
      if (!RNFS) {
        Alert.alert('Missing dependency', 'Install react-native-fs:\n\nnpm install react-native-fs');
        return;
      }
      tempPath = await dataUriToTempFile(dataUri, `ridesync-${Date.now()}.png`);
      if (!tempPath) { Alert.alert('Share failed', 'Could not write image file.'); return; }
      ...
      if (!e?.message?.includes('cancel') && e?.message !== 'User did not share') {
  ...
  const triggerSave = useCallback(async () => {
    ...
    try {
      ...
      if (!CameraRoll) {
        Alert.alert('Missing dependency', 'Install @react-native-camera-roll/camera-roll to save to gallery.');
        return;
      }

      const granted = await requestSavePermission();
      if (!granted) {
        Alert.alert('Permission denied', 'Please allow storage access in your device settings.');
        return;
      }

      const dataUri = await capture();
      if (!dataUri) { Alert.alert('Save failed', 'Could not render the card.'); return; }

      tempPath = await dataUriToTempFile(dataUri, `ridesync-${Date.now()}.png`);
      if (!tempPath) { Alert.alert('Save failed', 'Could not write image file.'); return; }

      await CameraRoll.saveAsset(tempPath, {type: 'photo', album: 'RideSync'});
      Alert.alert('Saved! 🎉', 'Your ride card has been saved to your gallery under the RideSync album.');
    } catch (e) {
